...
>
  <div className="card-header border-secondary d-flex justify-content-between align-items-center py-2 bg-black bg-opacity-25">
    <h5 id="layers-title" className="mb-0 fs-6 fw-bold">Layers</h5>
    <button 
      onClick={addLayer} 
      className="btn btn-primary btn-sm d-flex align-items-center gap-1"
      aria-label="Add layer"
    >
      <span aria-hidden="true">+</span> New Layer
    </button>
  </div>
        ...
        >
          <div className="d-flex align-items-center gap-2 mb-2">
            ...
            <div className="flex-grow-1">
              <label htmlFor={`opacity-${layer.id}`} className="visually-hidden">Opacity {layer.name}</label>
              <input 
                type="range" 
                id={`opacity-${layer.id}`}
